...
Программа использует JSON файл для настройки параметров мониторинга. Структура конфигурационного файла:
...
- **metrics**: Массив метрик для мониторинга.
  ...
  - **cpu_ids**: (только для CPU) Массив идентификаторов ядер процессора для мониторинга.
  - **spec**: (только для памяти) Массив параметров памяти для мониторинга (например, "MemTotal", "MemFree", "MemAvailable").
...
## 📝 Как составить конфигурационный файл
...
2. **Настройте период сбора метрик**:
   ...
   ```
   Здесь `5` - это период в секундах между сборами метрик.
...
3. **Добавьте метрики для мониторинга**:
   - Для CPU:
     ...
     ```
     Здесь `cpu_ids` - это массив идентификаторов ядер процессора, которые вы хотите мониторить.
   ...
   - Для памяти:
     ...
     ```
     Здесь `spec` - это массив параметров памяти, которые вы хотите отслеживать.
...
4. **Настройте выходы для данных**:
   ...
   - Для записи в файл:
     ...
     ```
     Здесь `path` - это путь к файлу, в который будут записываться метрики.

5. **Объедините все части в один файл**:
   ```json
   {
       "settings": {
           "period": 5
